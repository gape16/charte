										...
										<div class="single_choose_acording">
											<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
												<?php foreach (${"query_liste_".$key_categorie} as $key => $value) {
													...
													$query_commentaire_controle=$bdd->query("SELECT reponse_control FROM checked WHERE id_client = '$numero' AND id_question = '$questn'")->fetch();
													...
													<div class="panel panel-default cat_<?php echo $key_categorie;?>">
														...
														<div id="collapse<?php echo $key_categorie;?><?php echo $key;?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading<?php echo $key;?>" aria-expanded="false" style="height: auto;">
															<div class="panel-body">
																...
																<textarea class="reponse_<?php echo $value['id_liste'];?>" style="width: 100%;min-height:100px;" placeholder="Mettre un commentaire pour le graph"><?php echo utf8_encode($query_commentaire_controle['reponse_control']); ?></textarea>
															</div>
														</div>
													</div>
													<?php } ?>
													<textarea name="" id="" style="width: 100%;min-height:100px;" class="commentaire_<?php echo $key_categorie;?>" placeholder="commentaires"><?php echo utf8_encode($query_co['commentaire']);?></textarea><br><br>
													...
													<?php foreach (${"query_liste_".$key_categorie} as $key => $value) {
														...
														<div class="panel panel-default cat_<?php echo $key_categorie;?>">
															<div class="panel-heading" role="tab" id="heading<?php echo $key;?>">
																<h4 class="panel-title">
																	<a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo $key_categorie;?><?php echo $key;?>" aria-expanded="false" aria-controls="collapse<?php echo $key_categorie;?><?php echo $key;?>" class="collapsed">
																		<i class="fa <?php echo $value['icone'];?>" <?php if(utf8_encode($query_commentaire_controle['reponse_control'])!=""){?> style="color: #fff;
																			background: url(assets/images/ch_col_icn_h.png) no-repeat left center;" <?php }?>></i> <?php echo utf8_encode($value['titre']);?> 
																		</a>
																		<?php if($query_fini['fini']!=1){?>
																		<input type="checkbox" <?php if($query_que['reponse']==1){ echo "checked"; }?> class="question_<?php echo $value['id_liste'];?>">
																		<?php }else{?>
																		<input type="checkbox" disabled="disabled" <?php if($query_que['reponse']==1){ echo "checked"; }?> class="question_<?php echo $value['id_liste'];?>">
																		<?php }?>
																	</h4>
																</div>
																<div id="collapse<?php echo $key_categorie;?><?php echo $key;?>" class="panel-collapse collapse <?php if(utf8_encode($query_commentaire_controle['reponse_control'])!=""){?> in <?php }?>" role="tabpanel" aria-labelledby="heading<?php echo $key;?>" aria-expanded="false" style="height: auto;">
																	<div class="panel-body">
																		<?php echo utf8_encode($value['description']);?> <br>
																		<a href="<?php echo utf8_encode($value['lien_tuto']);?>">En savoir plus...</a><br><br>
																		<span class="commentaire"><?php echo utf8_encode($query_commentaire_controle['reponse_control']); ?></span>
																	</div>
																</div>
															</div>
															<?php } ?>
